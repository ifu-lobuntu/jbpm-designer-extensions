...
}if(c.error){try{var e=new Ext.form.TextArea({id:"errorSourceTextArea",fieldLabel:ORYX.I18N.main.errorDetails,value:c.error,autoScroll:true});
var d=new Ext.Panel({title:ORYX.I18N.main.details,layout:"fit",border:false,items:[e],collapsed:true,listeners:{expand:function(n){this.foldFunc=CodeMirror.newFoldFunction(CodeMirror.tagRangeFinder);
...
var l=new Ext.Panel({title:ORYX.I18N.File.info,layout:"table",border:false,bodyBorder:false,hideBorders:true,html:ORYX.I18N.main.errorOpening});
var b=new Ext.Panel({layout:"accordion",items:[l,d]});
var j=new Ext.Window({width:600,id:"processErrorSource",height:550,layout:"fit",title:ORYX.I18N.main.errorLoadingProc,items:[b],buttons:[{text:ORYX.I18N.Save.close,handler:function(){j.close();
j=null;
cf=null
}.bind(this)}]});
j.show()
...
return a
},getPersistentChildShapes:function(){var a=[];
this.children.each(function(b){if(b instanceof ORYX.Core.Shape&&(b.isVisible||b.behindCollapsedParent)){a.push(b)
}});
return a
...
d[c]=e;
return d
}.bind(this)),stencil:{id:this.getStencil().idWithoutNs()},childShapes:this.getPersistentChildShapes().map(function(b){return b.toJSON()
})};
if(this.getOutgoingShapes){a.outgoing=this.getOutgoingShapes().map(function(b){return{resourceId:b.resourceId}
})
}if(this.bounds){if(this.getStencil().id().endsWith("TextAnnotation")){a.bounds={lowerRight:this.absoluteBounds().lowerRight(),upperLeft:this.absoluteBounds().upperLeft()}
}else{a.bounds={lowerRight:this.bounds.lowerRight(),upperLeft:this.bounds.upperLeft()}
