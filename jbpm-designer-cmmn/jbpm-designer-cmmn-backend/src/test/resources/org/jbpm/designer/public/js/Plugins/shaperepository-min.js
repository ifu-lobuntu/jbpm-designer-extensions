...
e.each((function(j){try{if(j.hidden()){return
}var h=j.groups();
h.each((function(m){var l=m;
if(ORYX.I18N.propertyNames[m]&&ORYX.I18N.propertyNames[m].length>0){l=ORYX.I18N.propertyNames[m]
}if(!g[m]){if(Ext.isIE){g[m]=new Ext.tree.TreeNode({text:l,allowDrag:false,allowDrop:false,iconCls:"headerShapeRepImg",cls:"headerShapeRepChild",singleClickExpand:true,expanded:true});
g[m].expand()
}else{g[m]=new Ext.tree.TreeNode({text:l,allowDrag:false,allowDrop:false,iconCls:"headerShapeRepImg",cls:"headerShapeRepChild",singleClickExpand:true})
}b.appendChild(g[m]);
g[m].render()
}this.createStencilTreeNode(g[m],j)
}).bind(this));
if(h.length==0){this.createStencilTreeNode(b,j)
}var i=ORYX.CONFIG.STENCIL_GROUP_ORDER();
b.sort(function(m,l){return i[d.namespace()][m.text]-i[d.namespace()][l.text]
})
}catch(k){alert(k)
}}).bind(this))
}).bind(this))
