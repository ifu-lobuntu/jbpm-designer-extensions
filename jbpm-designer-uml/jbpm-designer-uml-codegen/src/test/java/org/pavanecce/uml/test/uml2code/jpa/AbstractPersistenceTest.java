...
import org.pavanecce.common.test.util.JavaCompilingTestHelper;

public abstract class AbstractPersistenceTest extends Assert {

	protected ConstructionCaseExample example;
	protected JavaCompilingTestHelper helper;

	public AbstractPersistenceTest(String name) {
		super();
		this.example=new ConstructionCaseExample(name);
		this.helper=new JavaCompilingTestHelper(example){};
	}
...
//			 eval("p.close();");
	...
	public void testSimpleTypes() throws Exception {
		...
		helper.getJavaScriptContext().setAttribute("date", date, ScriptContext.ENGINE_SCOPE);
		helper.getJavaScriptContext().setAttribute("picture", new byte[] { 1, 2, 3, 4, 5, 6 }, ScriptContext.ENGINE_SCOPE);
		eval("ConstructionCase=Packages.test.ConstructionCase;");
	...
	private Object eval(String string) throws ScriptException {
		return helper.getJavaScriptEngine().eval(string);
	}
